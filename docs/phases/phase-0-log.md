...
- Confirm the two AWS Budgets and the CloudWatch billing alarm are actually created in the AWS console (documentation is done in `docs/runbooks/aws-cost-guardrails.md`; the "Configured:" lines there are still blank pending confirmation).
...
- Review the constitution and Phase 1 spec, then proceed to `/speckit-plan` and `/speckit-tasks` to turn the spec into an implementation plan and task list.
